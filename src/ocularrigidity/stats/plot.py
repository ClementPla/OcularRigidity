...
def regression_plot_with_stats(
    ...
    if len(df) < 2:
        raise ValueError(
            f"{title!r}: only {len(df)} usable point(s) for '{x}' vs '{y}' "
    ...
    rho, p_val_sp = stats.spearmanr(df[x], df[y])
    ...
    x_pos, y_pos, va, ha = _ANNOT_POS.get(legend_position, _ANNOT_POS["best"])
    ax.text(
        x_pos,
        y_pos,
        annotation_text,
        ...
        ha=ha,
        bbox=dict(
            boxstyle="round,pad=0.4", facecolor="white", edgecolor="none", alpha=0.85
        ),
    )
